...
spec = 
        describe "zRepetition" $ do 
                ...
                it "repeats an operation with tool retraction" $
                        (runOperation defaultCuttingParameters (zRepetition (-2.3) (Just 1) (const $ approach (V3 0 0 0) >> feed (V3 10 0 0)))::IR) `shouldBe`
                ...
                it "repeats an operation without tool retraction" $
                        (runOperation defaultCuttingParameters (zRepetition (-2.3) Nothing (const $ approach (V3 0 0 0) >> feed (V3 10 0 0)))::IR) `shouldBe`
                            (runOperation defaultCuttingParameters $ do
                                let op = approach (V3 0 0 0) >> feed (V3 10 0 0)
                                translate (V3 0 0 (-0.5)) op
                                translate (V3 0 0 (-1)) op
