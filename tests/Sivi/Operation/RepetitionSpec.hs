...
spec = 
	describe "zRepetition" $ do 
		it "repeats an operation with tool retraction" $
			runOperation defaultCuttingParameters (zRepetition (-2.3) (Just 1) (approach (V3 0 0 0) +++ feed (V3 10 0 0))) `shouldBe`
		...
		it "repeats an operation with tool retraction" $
			runOperation defaultCuttingParameters (zRepetition (-2.3) Nothing (approach (V3 0 0 0) +++ feed (V3 10 0 0))) `shouldBe`
				IR [	Move (V3 0.0 0.0 0.0) Rapid,
					Move (V3 0.0 0.0 (-0.5)) (LinearInterpolation {feedRate = 30.0}),
					Move (V3 10.0 0.0 (-0.5)) (LinearInterpolation {feedRate = 100.0}),
					Move (V3 0.0 0.0 (-0.5)) Rapid,
