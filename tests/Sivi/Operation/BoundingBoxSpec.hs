...
spec = describe "boundingBox" $ do
	...
	it "calculates the bounding box of a square" $ 
		boundingBox' (square 30) `shouldBe` BoundingBox [Boundary (-16.5, 16.5), Boundary (-16.5, 16.5), Boundary (0, 0)]

	where
		boundingBox' op = runOperationWithDefaultParams $ boundingBox op
